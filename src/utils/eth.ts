...
export const CRYPTO_KITTIES_CONTRACT_ADDRESS = '0x06012c8cf97bead5deae237070f9587f8e7a266d'
...
const addHexPrefix = (value: string) => (startsWith(value, '0x') ? value : `0x${value}`)
...
const arrayToMapWithId = <T extends object>(array: T[], idKey: keyof T): ObjectMap<T> => {
...
const getShortenedAddress = (address: string, start: number = 6, end: number = 4) => {
...
export const toBaseUnitAmount = (amount: Numberish, decimals: number): BigNumber => {
  const unit = new BigNumber(BASE_TEN).pow(decimals)
  const baseUnitAmount = unit.times(amount)
  const hasDecimals = baseUnitAmount.decimalPlaces() !== 0
  if (hasDecimals) {
...
export const toNearestBaseUnitAmount = (amount: BigNumber, decimals: number): BigNumber => {
  const unit = new BigNumber(BASE_TEN).pow(decimals)
  const baseUnitAmount = unit.times(amount)
...
export const toBaseUnitAmountSafe = (
  ...
  return toBaseUnitAmount(new BigNumber(amount), new BigNumber(decimals).toNumber())
}
