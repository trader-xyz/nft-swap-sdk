...
import { assetDataUtils, signatureUtils as zeroExSigUtils } from '@0x/order-utils'
...
export type InterallySupportedAssetFormat = UserFacingSerializedSingleAssetDataTypes
...
export const signOrder = (
  ...
  if ((provider as any).isMetaMask) {
    stdProvider = new MetamaskSubprovider(provider as unknown as SupportedProvider)
  }
...
export const getApprovalStatus = async (
  ...
  switch (asset.type) {
    ...
    case 'ERC721':
      ...
      const erc721ApprovedAddressForIdPromise = erc721.getApproved(asset.tokenId)
      const [erc721ApprovalForAll, erc721ApprovedAddressForId] = await Promise.all([
        erc721ApprovalForAllPromise,
        erc721ApprovedAddressForIdPromise,
      ])
      const tokenIdApproved =
        erc721ApprovedAddressForId.toLowerCase() === exchangeProxyAddressForAsset.toLowerCase()
      return {
...
const getZeroExAddressesForChain = (chainId: number): AddressesForChain | undefined => {
...
export const getProxyAddressForErcType = (assetType: SupportedTokenTypes, chainId: number) => {
