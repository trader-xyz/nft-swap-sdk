...
interface INftSwap {
  signOrder: (order: Order, signerAddress: string, provider: BaseProvider) => Promise<SignedOrder>
  buildOrder: (
  ...
  fillSignedOrder: (signedOrder: SignedOrder, fillOrderOverrides?: Partial<FillOrderOverrides>) => Promise<string>
...
const convertAssetToInternalFormat = (swappable: SwappableAsset): InterallySupportedAssetFormat => {
...
class NftSwap implements INftSwap {
  ...
  constructor(provider: BaseProvider, chainId: ChainId, additionalConfig?: NftSwapConfig) {
  ...
  public loadApprovalStatus = async (
    ...
    const exchangeProxyAddressForAsset = getProxyAddressForErcType(asset.type as SupportedTokenTypes, this.chainId)
    return _getApprovalStatus(
      walletAddress,
      approvalOverrides?.exchangeProxyContractAddressForAsset ?? exchangeProxyAddressForAsset,
  ...
  public fillSignedOrder = async (
    ...
    return _sendSignedOrderToEthereum(signedOrder, fillOrverrides?.exchangeContract ?? this.exchangeContract)
  }
}
